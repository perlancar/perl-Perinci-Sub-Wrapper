...
subtest 'prop: args/as' => sub {
    my $meta = {v=>1.1,
                args=>{a=>{pos=>0, schema=>"int"},
                       b=>{pos=>1, schema=>"int"}}};
    {
        test_wrap(
            name => 'args/as=hash (default)',
    ...
    {
        local $meta->{args}{as} = 'hashref';
        test_wrap(
            name => 'args/as=hashref',
    ...
    {
        local $meta->{args}{as} = 'arrayref';
        test_wrap(
            name => 'args/as=arrayref',
    ...
    {
        local $meta->{args}{as} = 'hash';
        test_wrap(
            name => 'convert args/as hash -> array',
            wrap_args => {sub => sub {my %args=@_; [200,"OK",$args{a}/$args{b}]}, meta => $meta, convert=>{'args/as'=>'array'}},
            wrap_status => 200,
            calls => [
                {argsr=>[10, 5], status=>200, actual_res=>2},
                {argsr=>[12, 3], status=>200, actual_res=>4},
...

    {
        local $meta->{args}{as} = 'array';
        test_wrap(
            name => 'convert args/as array -> hash',
            wrap_args => {sub => sub {[200,"OK",$_[0]/$_[1]]}, meta => $meta, convert=>{'args/as'=>'hash'}},
            wrap_status => 200,
            calls => [
                {argsr=>[a=>10, b=>5], status=>200, actual_res=>2},
                {argsr=>[a=>12, b=>3], status=>200, actual_res=>4},
